...
export interface ToolRefinementNeeded {
  ...
  options: Array<{
    ...
    confidence: number;
    /**
     * Canonical selector id of the tool this option points to, when the option
     * was built from a concrete near-miss. Lets a caller re-dispatch the exact
     * tool deterministically (bypassing the embedder) instead of re-embedding a
     * label — the difference between a picker that resolves and one that loops.
     */
    canonical?: string;
  }>;
